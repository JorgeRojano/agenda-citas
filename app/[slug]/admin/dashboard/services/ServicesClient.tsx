...
import {
  ...
  TextInput, NumberInput, Stack, Title, Switch, Checkbox,
} from "@mantine/core";
import { useDisclosure } from "@mantine/hooks";
import { useForm } from "@mantine/form";
import { IconEdit, IconTrash, IconPlus, IconUsers } from "@tabler/icons-react";
import { useState, useEffect } from "react";
import { createService, updateService, deleteService, assignResourcesToService } from "./actions";
...
interface Props { business: Business; services: ServiceWithResources[] }
...
export default function ServicesClient({ business, services: initialServices }: Props) {
  const [services, setServices]         = useState<ServiceWithResources[]>(initialServices);
  ...
  const [resourcesService, setResourcesService] = useState<ServiceWithResources | null>(null);
  const [resourcesOpened, { open: openResources, close: closeResources }] = useDisclosure(false);
  ...
  const handleOpen = (service?: Service) => {
    if (service) {
      ...
      form.setValues({ name: service.name, duration: service.duration, price: service.price / 100, showPrice: service.showPrice });
    } else {
  ...
  const handleSaveResources = async () => {
    ...
    try {
      await assignResourcesToService(business.id, resourcesService.id, selectedResourceIds);
      setServices((prev) =>
        prev.map((s) =>
          s.id === resourcesService.id
            ? { ...s, resources: selectedResourceIds.map((id) => ({ profileId: id, profile: { id, name: allResources.find((r) => r.id === id)?.name ?? "" } })) }
            : s,
        ),
      );
      showNotification({ title: "Guardado", message: "Recursos actualizados", color: "green" });
      closeResources();
    } catch {
  ...
  const handleSubmit = async (values: typeof form.values) => {
    ...
    try {
      if (editingService) {
        ...
        setServices((prev) => prev.map((s) => s.id === editingService.id ? { ...s, ...values, price: values.price * 100 } : s));
    ...
    <>
      ...
      <SimpleGrid cols={{ base: 1, sm: 2, md: 3 }} spacing="md">
        {services.map((service) => (
          <Card key={service.id} withBorder radius="md" padding={0} shadow="sm" style={{ overflow: "hidden" }}>
            {/* Body */}
            <div style={{ padding: "16px 18px 12px" }}>
              <Text fw={700} size="md" mb={8}>{service.name}</Text>
              <Group gap="xs">
                <Badge variant="light" color="blue">⏱ {service.duration} min</Badge>
                {service.showPrice
                  ? <Badge variant="light" color="green">${(service.price / 100).toFixed(2)}</Badge>
        ...
        <Card
          ...
          style={{
            ...
            justifyContent: "center", minHeight: 160, background: "transparent",
          }}
        >
      ...
      {/* Modal crear/editar */}
      <Modal opened={opened} onClose={close} title={editingService ? "Editar servicio" : "Nuevo servicio"} centered>
        <form onSubmit={form.onSubmit(handleSubmit)}>
          <Stack gap="md">
            <TextInput label="Nombre" placeholder="Ej: Terapia de lenguaje" required {...form.getInputProps("name")} />
            <NumberInput label="Duración (minutos)" placeholder="60" min={1} required {...form.getInputProps("duration")} />
            <NumberInput label="Precio" placeholder="300" min={0} decimalScale={2} prefix="$" required {...form.getInputProps("price")} />
            <Switch
      ...
      <Modal
        opened={resourcesOpened} onClose={closeResources} centered
        title={
          <div>
            <Text fw={700} size="sm">Asignar recursos</Text>
            <Text size="xs" c="dimmed">{resourcesService?.name} · {resourcesService?.duration} min</Text>
          </div>
        }
      >
        <Stack gap="xs">
          <Text size="xs" c="dimmed" mb={4}>Selecciona los recursos que pueden realizar este servicio</Text>
          {allResources.length === 0 ? (
            <Text size="sm" c="dimmed" ta="center" py="md">
              No hay recursos configurados. Agrega recursos primero en la sección de Recursos.
            </Text>
          ) : (
            allResources.map((resource) => {
              const isSelected = selectedResourceIds.includes(resource.id as string);
              return (
                <div
                  key={resource.id}
                  ...
                  style={{
                    ...
                    background: isSelected ? "var(--mantine-color-violet-light)" : "var(--mantine-color-body)",
                    border: `1.5px solid ${isSelected ? "var(--mantine-color-violet-4)" : "var(--mantine-color-default-border)"}`,
                    transition: "all 0.1s",
                  }}
                >
                  <div style={{
                  ...
                  <div style={{ flex: 1 }}>
                    <Text size="sm" fw={700}>{resource.name}</Text>
                    {resource.specialty && <Text size="xs" c="dimmed">{resource.specialty}</Text>}
                  </div>
                  <Checkbox
                    checked={isSelected}
          ...
          <Group justify="flex-end" mt="sm">
            <Button variant="subtle" color="gray" onClick={closeResources}>Cancelar</Button>
            <Button color="violet" loading={savingResources} onClick={handleSaveResources}>Guardar</Button>
          </Group>
        </Stack>
      </Modal>

      {/* Modal eliminar */}
      <Modal opened={deleteOpened} onClose={closeDelete} title="Eliminar servicio" centered size="sm">
        <Stack gap="md">
